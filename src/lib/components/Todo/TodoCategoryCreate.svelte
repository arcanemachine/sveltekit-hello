<script lang="ts">
  ...
  function create() {
    ...
    const ids = $todoCategories.map((todoCategory) => todoCategory.id);
    ...
    $todoCategories = [...$todoCategories, todoCategory];

    textInputValue = ""; // clear the input field
    toastCreate(`Category created: '${todoCategory.name}'`, "success");
    modalClose();
  }
  ...
  <div class="modal">
    <div class="modal-box relative">
      ...
      <div class="flex-center mx-auto mt-6 flex w-full">
        ...
        <button class="btn-primary btn mx-2" on:click={create}>Create</button>
      </div>
    </div>
  </div>
{/if}
